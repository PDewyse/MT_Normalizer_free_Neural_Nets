...
class Squeeze_excite(nn.Module):
    def __init__(self, num_channels, r=4):
        ...
        #for example of this: https://sebastianraschka.com/faq/docs/fc-to-conv.html
        ...
        # note: maybe try to use normal activations instead of variance preserving ones but acount for a * 2 as in the paper

    def forward(self, input):
        out = self.conv_0(input)
        out = self.conv_1(out)# * 2 # as in the paper, see B.3 under squeeze and excitation layer
        out = out * input
        return out
...
class NFMobileNetv3(nn.Module):
    def __init__(self, wm=1.0, si=1, drop_prob=0.0, alpha=0.2,activation=Swish,signal_preserving=False,normalization=True):
        """
        ...
        """
        super(NFMobileNetv3, self).__init__()
        self.wm = wm
        ...
        #SE block is not present in the official tensorflow code [https://github.com/tensorflow/models/blob/master/research/slim/nets/mobilenet/mobilenet_v3.py]
        self.conv_2 = nn.Sequential(
