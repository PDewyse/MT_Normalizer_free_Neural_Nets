...
class MBConvBlock(nn.Module):
    def __init__(self, 
        ...
        if expand_ratio != 1:
            pointwise_conv1_layers = [nn.Conv2d(in_channels, expand_channels, 1, 1, 0, bias=False)]
            if normalization:
        ...
        depthwise_conv_layers = [nn.Conv2d(expand_channels,
                                           expand_channels,
                                           kernel_size,
                                           stride,
                                           kernel_size // 2,
        ...
        if se_ratio != 0:
            squeeze_excite = SqueezeAndExcite(expand_channels, in_channels, se_ratio, activation=activation, signal_preserving=signal_preserving)
            conv.append(squeeze_excite)

        # Projection phase
        pointwise_conv2_layers = [nn.Conv2d(expand_channels, out_channels, 1, 1, 0, bias=False)]
        if normalization:
    ...
    def forward(self, x):
        if self.residual_connection:
            return x + _drop_path(self.conv(x), self.drop_connect_rate, self.training)
        else:
            return self.conv(x)
...
class EfficientNet(nn.Module):
    def __init__(self, 
        ...
        # head convolution
        self.head = [nn.Conv2d(in_channels=config[-1][1], 
