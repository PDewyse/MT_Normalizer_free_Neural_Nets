...
class CNN1(nn.Module):
    def __init__(self,
        ...
        self.classifier = nn.Sequential(
            ...
            *([nn.BatchNorm1d(feature_size)] if self.normalization else []),
            activation(signal_preserving=signal_preserving),
            nn.Linear(feature_size, int(feature_size/2)),
            *([nn.BatchNorm1d(int(feature_size/2))] if self.normalization else []),
            activation(signal_preserving=signal_preserving),
...
if __name__ == "__main__":
    ...
    model = CNN1(activation=activation_class, signal_preserving=True, normalization=True)
